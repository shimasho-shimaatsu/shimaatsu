...
var DescriptionModel = function(data) {
  ...
  this.label = data[5];
  this.sublabel = data[6];//not used
  this.description = data[7];//not used
  this.styles = data[8];
  this.background = data[9];
  ...
  function updateAreaList() {
    csvToArray("data/area_days.csv", function(tmp) {
      ...
      for (var i in tmp) {
        ...
        //２列目以降の処理
        for (var r = 10; r < 10 + MaxDescription; r++) {
      ...
      csvToArray("data/target.csv", function(data) {
        ...
        for (var i in data) {
          ...
          for (var j = 0; j < descriptions.length; j++) {
            //一致してるものに追加する。
            if (descriptions[j].label == row.area) {
